...
rc('text', usetex=True)
mof = read('CONTCAR')
Rh = [atom.index for atom in mof if atom.symbol == 'Rh']
...
pdos = VaspXML.dos_parametric(atoms=Rh, spin=[0,1])
for i, spin in enumerate(spins):
	y = pdos.dos[:, i+1]*spin
	label = 'Rh' if i == 0 else None
...
ax[2].legend(loc='upper right',ncol=3,columnspacing=1)
ax[2].set_xlim(elim)
ax[2].set_ylim([-50,50])
